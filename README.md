...
When a file `sync` manages was edited locally, `sync` prompts (on a TTY) to replace it or
keep the edit — `--no-interactive` (or a non-TTY `stdin`, e.g. in CI) always replaces without
